...
const handleSubmit = async () => {
  ...
  try {
    if (showModal === "add") {
      ...
      await axiosInstance.put(`/auth/api/update-address/${selectedAddress.id}`, formData);
...
<ScrollView className="flex-1 p-4" showsVerticalScrollIndicator={false}>
  {showModal === "delete" ? (
    ...
    <View className="flex-1 justify-center items-center py-20">
      ...
      <Text className="text-gray-600 font-poppins-medium text-center mb-4">
...
<View className="w-full gap-3">
  ...
  <TouchableOpacity
    ...
    onPress={() => selectedAddress && handleDeleteAddress(selectedAddress)}
  >
  ...
  {filteredCountries.map((country) => (
    <TouchableOpacity
      ...
      className={`p-4 border-b border-gray-100 ${
        formData.country === country ? "bg-blue-50" : "bg-white"
      }`}
      onPress={() => {
...
<View className="flex-row items-center justify-between py-3">
  ...
  onPress={() => {
    // If trying to uncheck default and this is the only default address
    if (
      formData.isDefault &&
      showModal === "edit" &&
      selectedAddress
...
>
  <Text className="text-white font-poppins-semibold text-center">
    {isSubmitting
      ? "Saving..."
      : (showModal === "add" ? "Add Address" : "Update Address")}
  </Text>
</TouchableOpacity>
  </View>
    )}
...
<TouchableOpacity
  className="bg-white rounded-2xl shadow-[0_0_1px_rgba(0,0,0,0.1)] border border-gray-300 p-6 items-center"
  onPress={openAddModal}
  activeOpacity={0.7}
>
