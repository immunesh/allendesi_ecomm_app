...
export default function ShopDetailsScreen() {
  ...
  const productsLoading = shopLoading;
  ...
  const handleFollowToggle = async () => {
    if (!user) {
      Toast.show({ type: "error", text1: "Please login to follow shops" });
      return;
    }

    if (!shop?.id) {
      Toast.show({ type: "error", text1: "Unable to update follow status." });
      return;
    }
    ...
    try {
      ...
      Toast.show({ type: "error", text1: "Failed to update follow status. Please try again." });
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleChat = async () => {
    ...
    try {
      ...
      console.log(error);
      Toast.show({ type: "error", text1: "Failed to start chat. Please try again." });
    } finally {
  ...
  const renderHeader = () => (
    <View>
          ...
          <View className="mt-6 flex-row gap-4">
            ...
            >
              <View className="flex-row items-center justify-center">
                ...
                <Text className="text-center font-poppins-bold text-base text-white">
                  {isChatLoading ? "..." : "Chat"}
                </Text>
              </View>
            </TouchableOpacity>
      ...
      <View className="px-4 mt-6">
        <View className="bg-white rounded-2xl p-6 shadow-[0_0_3px_rgba(0,0,0,0.1)]">
  ...
  const renderProductItem = ({ item }: { item: any }) => (
    <TouchableOpacity
      className="bg-white rounded-2xl shadow-lg mb-4 overflow-hidden border"
  ...
  const renderTabContent = () => {
      ...
      return (
        <View className="px-4 mt-6">
          {productsLoading ? (
            ...
            <FlatList
              data={products?.filter((product: any) => !product.starting_date)}
    ...
    if (activeTab === "Offers") {
      return (
        <View className="px-4 mt-6">
          <View className="bg-white rounded-2xl p-6 shadow-[0_0_3px_rgba(0,0,0,0.1)]">
            <Text className="text-lg font-poppins-bold text-gray-900 mb-2">
              Shop Offers
            </Text>
    ...
    return (
      <View className="px-4 mt-6">
        <View className="bg-white rounded-xl p-6 shadow-[0_0_3px_rgba(0,0,0,0.1)]">
